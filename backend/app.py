    ...
    def get_feedbacks():
        ...
        try:
            with open(log_path, "r", encoding="utf-8") as f:
                ...
                import re
                # Split by different possible headers
                blocks = re.split(r'--- NEW FEEDBACK|🚀 NEW FEEDBACK|={50}', content)
                ...
                for block in blocks:
                    ...
                    for line in lines:
                        ...
                        if "MESSAGE:" in line: 
                            msg_part = line.split("MESSAGE:")[1].strip()
                            entry['message'] = msg_part
                        
                        if "(" in line and ")" in line and not entry.get('date'): 
                            entry['date'] = line.split("(")[1].split(")")[0]
                        elif "AT " in line and not entry.get('date'):
                            entry['date'] = line.split("AT ")[1].strip()
